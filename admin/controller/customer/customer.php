...
class ControllerCustomerCustomer extends Controller {
	...
	public function import(){
		...
		$this->document->setTitle($this->language->get('heading_title'));
		$this->load->model('customer/customer');
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && isset($this->request->files) && $this->request->files['file']['tmp_name'] !="") {
			
			$this->model_customer_customer->addSpecialPrice($this->request->get['customer_id'], $this->request->files);
		}else{
			$this->model_customer_customer->getSpecialPrices($this->request->get['customer_id']);
		}
	...
	public function updateImport(){
		$this->load->model('customer/customer');
		$this->model_customer_customer->updateSpecialPrices($this->request->get['row_id'],$this->request->get['column_id'],$this->request->get['value']);
	}

	public function deleteImport(){
		$this->load->model('customer/customer');
		$this->model_customer_customer->deleteSpecialPrices($this->request->get['row_id']);
	}
	...
	protected function getForm() {
		...
		$data["special_prices"] = $this->model_customer_customer->getSpecialPrices($this->request->get['customer_id']);
