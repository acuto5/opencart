...
class ModelCustomerCustomer extends Model {
	...
	public function addSpecialPrice($customer_id, $data){
		$filename=$_FILES["file"]["tmp_name"];	
		$file_parts = pathinfo($filename);
		if($_FILES["file"]["size"] > 0){
			$file = fopen($filename, "r");
			$this->db->query("DELETE FROM " . DB_PREFIX . "customer_special_prices WHERE customer_id=" . $this->request->get['customer_id']);
			while (($getData = fgetcsv($file, 10000, ",")) !== FALSE){
				$values = explode("|", $getData[0]);
				print_r($values);
				$this->db->query("INSERT INTO " . DB_PREFIX . "customer_special_prices (customer_id, ean, special_price, added_by) VALUES ($customer_id, $values[0], $values[1], " . $this->session->data['user_id'] . ")");
			}
			fclose($file);	
		}
	}
	
	public function deleteSpecialPrices($id){
		$q ="DELETE FROM " . DB_PREFIX . "customer_special_prices WHERE id=$id";
		$this->db->query($q);
	}
	
	public function getSpecialPrices($customer_id){
		$q = "SELECT " . DB_PREFIX . "product.model, " 
			. DB_PREFIX . "product.product_id, "
			. DB_PREFIX . "user.username, "
			. DB_PREFIX . "customer_special_prices.id, " 
			. DB_PREFIX . "customer_special_prices.ean, "
			. DB_PREFIX . "customer_special_prices.special_price, "
			. DB_PREFIX . "customer_special_prices.date_added "
			
			. "FROM " . DB_PREFIX . "customer_special_prices " 
			. "INNER JOIN " . DB_PREFIX . "product ON " . DB_PREFIX . "product.ean=" .  DB_PREFIX . "customer_special_prices.ean "
			. "INNER JOIN " . DB_PREFIX . "user ON " . DB_PREFIX . "user.user_id=" .  DB_PREFIX . "customer_special_prices.added_by " 
			. "WHERE " . DB_PREFIX . "customer_special_prices.customer_id=" . $customer_id;
		$query = $this->db->query($q);
		return $query->rows;
	}
	
	public function updateSpecialPrices($row_id, $column_id, $value){
		$q = "UPDATE " . DB_PREFIX . "customer_special_prices SET $column_id='$value' WHERE id=$row_id";
		$this->db->query($q);
	}
